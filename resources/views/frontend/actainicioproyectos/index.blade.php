        ...
        <div class="col-md-12">
            @can('actainicioproyecto_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.actainicioproyectos.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-Actainicioproyecto">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $actainicioproyecto->id }}">
                                        ...
                                        <td>
                                            @can('actainicioproyecto_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.actainicioproyectos.show', $actainicioproyecto->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('actainicioproyecto_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.actainicioproyectos.edit', $actainicioproyecto->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('actainicioproyecto_delete')
                                                <form action="{{ route('frontend.actainicioproyectos.destroy', $actainicioproyecto->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('actainicioproyecto_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.actainicioproyectos.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-Actainicioproyecto:not(.ajaxTable)').DataTable({ buttons: dtButtons })
