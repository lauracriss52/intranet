                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.procesos.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('procedimientos') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="procedimientos[]" id="procedimientos" multiple>
                                @foreach($procedimientos as $id => $procedimiento)
                                    <option value="{{ $id }}" {{ in_array($id, old('procedimientos', [])) ? 'selected' : '' }}>{{ $procedimiento }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('lider_del_proceso') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="lider_del_proceso" id="lider_del_proceso" value="{{ old('lider_del_proceso', '') }}">
