        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.presupuesto.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.presupuestos.update", [$presupuesto->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('nombre') ? 'has-error' : '' }}">
                            <label class="required" for="nombre">{{ trans('cruds.presupuesto.fields.nombre') }}</label>
                            <input class="form-control" type="text" name="nombre" id="nombre" value="{{ old('nombre', $presupuesto->nombre) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('descripcion') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="descripcion" id="descripcion" value="{{ old('descripcion', $presupuesto->descripcion) }}">
                        ...
                        <div class="form-group {{ $errors->has('valor') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="valor" id="valor" value="{{ old('valor', $presupuesto->valor) }}" step="1" required>
                            @if($errors->has('valor'))
                        ...
                        <div class="form-group {{ $errors->has('sgi') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="sgi_id" id="sgi_id">
                                @foreach($sgis as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('sgi_id') ? old('sgi_id') : $presupuesto->sgi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('sgi'))
                        ...
                        <div class="form-group {{ $errors->has('proceso') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="proceso_id" id="proceso_id" required>
                                @foreach($procesos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('proceso_id') ? old('proceso_id') : $presupuesto->proceso->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
