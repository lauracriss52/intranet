        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.terminacionContrato.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.terminacion-contratos.update", [$terminacionContrato->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('fecha_terminacion') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha_terminacion" id="fecha_terminacion" value="{{ old('fecha_terminacion', $terminacionContrato->fecha_terminacion) }}">
                        ...
                        <div class="form-group {{ $errors->has('empleado') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="empleado_id" id="empleado_id">
                                @foreach($empleados as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('empleado_id') ? old('empleado_id') : $terminacionContrato->empleado->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
