        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.documentosEmpleadoo.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.documentos-empleadoos.update", [$documentosEmpleadoo->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('tipo_de_documento') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="tipo_de_documento" id="tipo_de_documento" required>
                                ...
                                @foreach(App\Models\DocumentosEmpleadoo::TIPO_DE_DOCUMENTO_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('tipo_de_documento', $documentosEmpleadoo->tipo_de_documento) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('fecha') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha" id="fecha" value="{{ old('fecha', $documentosEmpleadoo->fecha) }}">
                            @if($errors->has('fecha'))
                        ...
                        <div class="form-group {{ $errors->has('nombre') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="nombre" id="nombre" value="{{ old('nombre', $documentosEmpleadoo->nombre) }}" required>
