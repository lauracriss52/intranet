        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.actaJuntum.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.acta-junta.update", [$actaJuntum->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('fecha') ? 'has-error' : '' }}">
                            <label class="required" for="fecha">{{ trans('cruds.actaJuntum.fields.fecha') }}</label>
                            <input class="form-control date" type="text" name="fecha" id="fecha" value="{{ old('fecha', $actaJuntum->fecha) }}" required>
                            @if($errors->has('fecha'))
                        ...
                        <div class="form-group {{ $errors->has('hora_inicio') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control timepicker" type="text" name="hora_inicio" id="hora_inicio" value="{{ old('hora_inicio', $actaJuntum->hora_inicio) }}">
                        ...
                        <div class="form-group {{ $errors->has('hora_final') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control timepicker" type="text" name="hora_final" id="hora_final" value="{{ old('hora_final', $actaJuntum->hora_final) }}">
                        ...
                        <div class="form-group {{ $errors->has('asunto') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="asunto" id="asunto" value="{{ old('asunto', $actaJuntum->asunto) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('proceso') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="proceso_id" id="proceso_id">
                                @foreach($procesos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('proceso_id') ? old('proceso_id') : $actaJuntum->proceso->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('modalidad') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\ActaJuntum::MODALIDAD_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="modalidad_{{ $key }}" name="modalidad" value="{{ $key }}" {{ old('modalidad', $actaJuntum->modalidad) === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('descripcion') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="descripcion" id="descripcion">{!! old('descripcion', $actaJuntum->descripcion) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('conclusiones') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="conclusiones" id="conclusiones">{!! old('conclusiones', $actaJuntum->conclusiones) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('compromisos') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="compromisos" id="compromisos">{!! old('compromisos', $actaJuntum->compromisos) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('elaboro') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="elaboro_id" id="elaboro_id">
                                @foreach($elaboros as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('elaboro_id') ? old('elaboro_id') : $actaJuntum->elaboro->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('asistentes') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="asistentes[]" id="asistentes" multiple required>
                                @foreach($asistentes as $id => $asistente)
                                    <option value="{{ $id }}" {{ (in_array($id, old('asistentes', [])) || $actaJuntum->asistentes->contains($id)) ? 'selected' : '' }}>{{ $asistente }}</option>
                                @endforeach
                            </select>
