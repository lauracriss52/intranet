        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.cocolab.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.cocolabs.update", [$cocolab->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('nombre') ? 'has-error' : '' }}">
                            <label class="required" for="nombre">{{ trans('cruds.cocolab.fields.nombre') }}</label>
                            <input class="form-control" type="text" name="nombre" id="nombre" value="{{ old('nombre', $cocolab->nombre) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('fecha') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha" id="fecha" value="{{ old('fecha', $cocolab->fecha) }}">
                            @if($errors->has('fecha'))
                        ...
                        <div class="form-group {{ $errors->has('empleados') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="empleados[]" id="empleados" multiple required>
                                @foreach($empleados as $id => $empleado)
                                    <option value="{{ $id }}" {{ (in_array($id, old('empleados', [])) || $cocolab->empleados->contains($id)) ? 'selected' : '' }}>{{ $empleado }}</option>
                                @endforeach
                            </select>
