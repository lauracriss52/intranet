        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.expedientesCocolab.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.expedientes-cocolabs.update", [$expedientesCocolab->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('fecha') ? 'has-error' : '' }}">
                            <label for="fecha">{{ trans('cruds.expedientesCocolab.fields.fecha') }}</label>
                            <input class="form-control date" type="text" name="fecha" id="fecha" value="{{ old('fecha', $expedientesCocolab->fecha) }}">
                            @if($errors->has('fecha'))
                        ...
                        <div class="form-group {{ $errors->has('descripcion_expediente') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="descripcion_expediente" id="descripcion_expediente" value="{{ old('descripcion_expediente', $expedientesCocolab->descripcion_expediente) }}" required>
