        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.actaentrega.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.actaentregas.update", [$actaentrega->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('fecha') ? 'has-error' : '' }}">
                            <label class="required" for="fecha">{{ trans('cruds.actaentrega.fields.fecha') }}</label>
                            <input class="form-control date" type="text" name="fecha" id="fecha" value="{{ old('fecha', $actaentrega->fecha) }}" required>
                            @if($errors->has('fecha'))
                        ...
                        <div class="form-group {{ $errors->has('tipo_de_entregas') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="tipo_de_entregas[]" id="tipo_de_entregas" multiple required>
                                @foreach($tipo_de_entregas as $id => $tipo_de_entrega)
                                    <option value="{{ $id }}" {{ (in_array($id, old('tipo_de_entregas', [])) || $actaentrega->tipo_de_entregas->contains($id)) ? 'selected' : '' }}>{{ $tipo_de_entrega }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('ciudad') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="ciudad" id="ciudad" value="{{ old('ciudad', $actaentrega->ciudad) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('modalidad') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Actaentrega::MODALIDAD_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="modalidad_{{ $key }}" name="modalidad" value="{{ $key }}" {{ old('modalidad', $actaentrega->modalidad) === (string) $key ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('gestions') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="gestions[]" id="gestions" multiple required>
                                @foreach($gestions as $id => $gestion)
                                    <option value="{{ $id }}" {{ (in_array($id, old('gestions', [])) || $actaentrega->gestions->contains($id)) ? 'selected' : '' }}>{{ $gestion }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('observaciones') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="observaciones" id="observaciones">{!! old('observaciones', $actaentrega->observaciones) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('responsabilidades') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="responsabilidades" id="responsabilidades">{!! old('responsabilidades', $actaentrega->responsabilidades) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('elaboro') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="elaboro_id" id="elaboro_id" required>
                                @foreach($elaboros as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('elaboro_id') ? old('elaboro_id') : $actaentrega->elaboro->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('recibe') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="recibe_id" id="recibe_id" required>
                                @foreach($recibes as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('recibe_id') ? old('recibe_id') : $actaentrega->recibe->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
