        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.salidaCampo.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.salida-campos.update", [$salidaCampo->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('fecha_de_ingreso') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha_de_ingreso" id="fecha_de_ingreso" value="{{ old('fecha_de_ingreso', $salidaCampo->fecha_de_ingreso) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('fecha_de_salida') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha_de_salida" id="fecha_de_salida" value="{{ old('fecha_de_salida', $salidaCampo->fecha_de_salida) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('cliente') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="cliente" id="cliente" required>
                                ...
                                @foreach(App\Models\SalidaCampo::CLIENTE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('cliente', $salidaCampo->cliente) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('empleados') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="empleados[]" id="empleados" multiple required>
                                @foreach($empleados as $id => $empleado)
                                    <option value="{{ $id }}" {{ (in_array($id, old('empleados', [])) || $salidaCampo->empleados->contains($id)) ? 'selected' : '' }}>{{ $empleado }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('transporte_casa_aeropuerto') ? 'has-error' : '' }}">
                            <div>
                                <input type="checkbox" name="transporte_casa_aeropuerto" id="transporte_casa_aeropuerto" value="1" {{ $salidaCampo->transporte_casa_aeropuerto || old('transporte_casa_aeropuerto', 0) === 1 ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('transporte_11') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="transporte_11" id="transporte_11" value="{{ old('transporte_11', $salidaCampo->transporte_11) }}">
                        ...
                        <div class="form-group {{ $errors->has('transporte_2') ? 'has-error' : '' }}">
                            <div>
                                <input type="checkbox" name="transporte_2" id="transporte_2" value="1" {{ $salidaCampo->transporte_2 || old('transporte_2', 0) === 1 ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('transporte_22') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="transporte_22" id="transporte_22" value="{{ old('transporte_22', $salidaCampo->transporte_22) }}">
                        ...
                        <div class="form-group {{ $errors->has('transporte_3') ? 'has-error' : '' }}">
                            <div>
                                <input type="checkbox" name="transporte_3" id="transporte_3" value="1" {{ $salidaCampo->transporte_3 || old('transporte_3', 0) === 1 ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('transporte_4') ? 'has-error' : '' }}">
                            <div>
                                <input type="checkbox" name="transporte_4" id="transporte_4" value="1" {{ $salidaCampo->transporte_4 || old('transporte_4', 0) === 1 ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('transporte_5') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="transporte_5" id="transporte_5" value="1" {{ $salidaCampo->transporte_5 || old('transporte_5', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('detalle') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="detalle" id="detalle">{!! old('detalle', $salidaCampo->detalle) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('projecto') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="projecto_id" id="projecto_id">
                                @foreach($projectos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('projecto_id') ? old('projecto_id') : $salidaCampo->projecto->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('proveedors') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="proveedors[]" id="proveedors" multiple>
                                @foreach($proveedors as $id => $proveedor)
                                    <option value="{{ $id }}" {{ (in_array($id, old('proveedors', [])) || $salidaCampo->proveedors->contains($id)) ? 'selected' : '' }}>{{ $proveedor }}</option>
                                @endforeach
                            </select>
