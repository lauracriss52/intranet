        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.odsCompraAuditorium.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.ods-compra-auditoria.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('orden_de_servicio') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="orden_de_servicio_id" id="orden_de_servicio_id">
                                @foreach($orden_de_servicios as $id => $entry)
                                    <option value="{{ $id }}" {{ old('orden_de_servicio_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('proveedor') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="proveedor" id="proveedor" value="1" {{ old('proveedor', 0) == 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('descuentos') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="descuentos" id="descuentos" value="1" {{ old('descuentos', 0) == 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('pago') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="pago" id="pago" value="1" {{ old('pago', 0) == 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('factura') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="factura" id="factura" value="1" {{ old('factura', 0) == 1 ? 'checked' : '' }}>
