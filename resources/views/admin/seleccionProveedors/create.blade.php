        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.seleccionProveedor.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.seleccion-proveedors.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('ods') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="ods_id" id="ods_id">
                                @foreach($ods as $id => $entry)
                                    <option value="{{ $id }}" {{ old('ods_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('ods'))
                        ...
                        <div class="form-group {{ $errors->has('criterio_1') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="criterio_1" id="criterio_1" value="{{ old('criterio_1', '') }}">
                        ...
                        <div class="form-group {{ $errors->has('criterio_2') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="criterio_2" id="criterio_2" value="{{ old('criterio_2', '') }}">
                        ...
                        <div class="form-group {{ $errors->has('empresa_seleccionada') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="empresa_seleccionada_id" id="empresa_seleccionada_id">
                                @foreach($empresa_seleccionadas as $id => $entry)
                                    <option value="{{ $id }}" {{ old('empresa_seleccionada_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
