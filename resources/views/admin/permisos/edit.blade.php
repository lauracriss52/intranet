        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.permiso.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.permisos.update", [$permiso->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('tipo_de_permiso') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="tipo_de_permiso" id="tipo_de_permiso" required>
                                ...
                                @foreach(App\Models\Permiso::TIPO_DE_PERMISO_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('tipo_de_permiso', $permiso->tipo_de_permiso) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('duracion') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="duracion" id="duracion" value="{{ old('duracion', $permiso->duracion) }}" step="0.1" required>
                        ...
                        <div class="form-group {{ $errors->has('fecha') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha" id="fecha" value="{{ old('fecha', $permiso->fecha) }}" required>
                            @if($errors->has('fecha'))
