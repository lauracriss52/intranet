        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.dotacion.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.dotacions.update", [$dotacion->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('cantidad') ? 'has-error' : '' }}">
                            <label class="required" for="cantidad">{{ trans('cruds.dotacion.fields.cantidad') }}</label>
                            <input class="form-control" type="number" name="cantidad" id="cantidad" value="{{ old('cantidad', $dotacion->cantidad) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('tipo') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="tipo" id="tipo" required>
                                ...
                                @foreach(App\Models\Dotacion::TIPO_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('tipo', $dotacion->tipo) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('tipo'))
                        ...
                        <div class="form-group {{ $errors->has('talla') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="talla" id="talla" value="{{ old('talla', $dotacion->talla) }}" required>
                            @if($errors->has('talla'))
                        ...
                        <div class="form-group {{ $errors->has('genero') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Dotacion::GENERO_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="genero_{{ $key }}" name="genero" value="{{ $key }}" {{ old('genero', $dotacion->genero) === (string) $key ? 'checked' : '' }} required>
