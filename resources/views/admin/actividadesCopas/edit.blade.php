        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.actividadesCopa.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.actividades-copas.update", [$actividadesCopa->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('nombre') ? 'has-error' : '' }}">
                            <label class="required" for="nombre">{{ trans('cruds.actividadesCopa.fields.nombre') }}</label>
                            <input class="form-control" type="text" name="nombre" id="nombre" value="{{ old('nombre', $actividadesCopa->nombre) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('fecha') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha" id="fecha" value="{{ old('fecha', $actividadesCopa->fecha) }}">
                            @if($errors->has('fecha'))
                        ...
                        <div class="form-group {{ $errors->has('empleados') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="empleados[]" id="empleados" multiple required>
                                @foreach($empleados as $id => $empleado)
                                    <option value="{{ $id }}" {{ (in_array($id, old('empleados', [])) || $actividadesCopa->empleados->contains($id)) ? 'selected' : '' }}>{{ $empleado }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('lista_de_asistencia') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="lista_de_asistencia_id" id="lista_de_asistencia_id">
                                @foreach($lista_de_asistencias as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('lista_de_asistencia_id') ? old('lista_de_asistencia_id') : $actividadesCopa->lista_de_asistencia->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('acta') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="acta_id" id="acta_id">
                                @foreach($actas as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('acta_id') ? old('acta_id') : $actividadesCopa->acta->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('acta'))
