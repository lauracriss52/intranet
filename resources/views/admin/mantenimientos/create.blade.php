        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.mantenimiento.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.mantenimientos.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('fecha_programacion') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha_programacion" id="fecha_programacion" value="{{ old('fecha_programacion') }}">
                        ...
                        <div class="form-group {{ $errors->has('fecha_de_ejecucion') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha_de_ejecucion" id="fecha_de_ejecucion" value="{{ old('fecha_de_ejecucion') }}">
                        ...
                        <div class="form-group {{ $errors->has('dispositivo') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="dispositivo_id" id="dispositivo_id">
                                @foreach($dispositivos as $id => $entry)
                                    <option value="{{ $id }}" {{ old('dispositivo_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
