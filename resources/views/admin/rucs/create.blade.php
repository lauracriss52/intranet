        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.ruc.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.rucs.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('documento_solicitado') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="documento_solicitado" id="documento_solicitado" value="{{ old('documento_solicitado', '') }}">
                        ...
                        <div class="form-group {{ $errors->has('estado') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Ruc::ESTADO_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="estado_{{ $key }}" name="estado" value="{{ $key }}" {{ old('estado', '') === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('lista_asistencia') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="lista_asistencia_id" id="lista_asistencia_id">
                                @foreach($lista_asistencias as $id => $entry)
                                    <option value="{{ $id }}" {{ old('lista_asistencia_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('listado_maestro') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="listado_maestro_id" id="listado_maestro_id">
                                @foreach($listado_maestros as $id => $entry)
                                    <option value="{{ $id }}" {{ old('listado_maestro_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
