        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.listadomaestro.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.listadomaestros.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('codigo_del_documento') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="codigo_del_documento" id="codigo_del_documento" value="{{ old('codigo_del_documento', '') }}" required>
                        ...
                        <div class="form-group {{ $errors->has('version') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="version" id="version" value="{{ old('version', '1') }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('tipodocumento') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="tipodocumento_id" id="tipodocumento_id" required>
                                @foreach($tipodocumentos as $id => $entry)
                                    <option value="{{ $id }}" {{ old('tipodocumento_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('estado') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Listadomaestro::ESTADO_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="estado_{{ $key }}" name="estado" value="{{ $key }}" {{ old('estado', '') === (string) $key ? 'checked' : '' }} required>
